...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        n = imgs.shape[0]
        img_size = imgs.shape[2:]   # [800, 800]
        
        # 获取公用特征层
        base_feature = self.faster_rcnn.extractor(imgs)   # 特征map的维度：[2, 1024, 50, 50]
        print('base_feature.shape', base_feature.shape)
        ...
        for i in range(n):
            bbox = bboxes[i]  # bbox 是一个batch图片的真实标签
            label = labels[i]  # label是一个batch图片的真是类别标签
            rpn_loc = rpn_locs[i]  # 取出rpn
            rpn_score = rpn_scores[i]
            roi = rois[roi_indices==i] # 取出该batch中第i张图片的最终的600个rois
            feature = base_feature[i]  # 取出该batch中第i张图片的共享特征
            ...
            # -------------------------------------------------- #
            gt_rpn_loc, gt_rpn_label = self.anchor_target_creator(bbox, anchor, img_size)  # bbox真实框，anchor先验框，
            ...
            # -------------------------------------------------- #
            #   分别计算建议框网络的回归损失和分类损失， rpn_loc是base_feature经过回归分支得到的结果  gt_rpn_loc是proposal和与proposal一一对应的最匹配的真实bbox的偏差，
            # -------------------------------------------------- #
            rpn_loc_loss = _fast_rcnn_loc_loss(rpn_loc, gt_rpn_loc, gt_rpn_label, self.rpn_sigma)  # 回归损失，只计算正样本（前景）的损失，不计算负样本的位置损失。
            ...
            #   gt_roi_label    [n_sample, ]   # roi 是该图片前600个分数最高的的proposal（用NMS得到的）
            # ------------------------------------------------------ #
            ...
            print('roi_score.shape', roi_score.shape)
            ...
            roi_cls_loss = nn.CrossEntropyLoss()(roi_score[0], gt_roi_label)  # 使用 CrossEntropyLoss 不需要经过softmax
...
def _fast_rcnn_loc_loss(pred_loc, gt_loc, gt_label, sigma):
    pred_loc = pred_loc[gt_label>0]  # 将正样本对应的pred_loc找出来
    gt_loc = gt_loc[gt_label>0]  # 将正样本对应的gt_loc找出来

    loc_loss = _smooth_l1_loss(pred_loc, gt_loc, sigma)   # 边框回归损失用 smooth_l1_loss
