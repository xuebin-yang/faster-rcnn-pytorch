...
def fit_ont_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):   # batch.shape = [2, 3, 800, 800]
...
if __name__ == "__main__":
    ...
    np.random.shuffle(lines)   # 打乱图片顺序
    np.random.seed(None)
    num_val = int(len(lines)*val_split)  # 得到验证集的数量
    num_train = len(lines) - num_val  # 得到训练集图像数量
    ...
    if True:
        ...
        epoch_size      = num_train // Batch_size  # 一个 epoch 要用多少个 batch-size 才能读完所有数据
        epoch_size_val  = num_val // Batch_size    # 一个 epoch 要用多少个 batch-size 才能读完验证集所有图片
        ...
        #   冻结一定部分训练
        # ------------------------------------#
        for param in model.extractor.parameters():   # 将特征提取的网络参数冻结
            param.requires_grad = False

        # ------------------------------------#
        #   冻结bn层
        # ------------------------------------#
        model.freeze_bn()   #冻结BN层的参数

        train_util      = FasterRCNNTrainer(model, optimizer)   # 实例化 FasterRCNNTrainer 这个类

        for epoch in range(Init_Epoch,Freeze_Epoch):  # Freeze_Epoch为冻结训练的世代
    ...
    if True:
        ...
        train_util      = FasterRCNNTrainer(model,optimizer)  # 类的实例化
